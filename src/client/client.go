...
type Session interface {
	...
	SavePacket(clientsession.Direction, packet.GenericPacket) error

	// LookupPacket will retrieve a packet from the session using a packet id.
	LookupPacket(clientsession.Direction, packet.ID) (packet.GenericPacket, error)
	...
	DeletePacket(clientsession.Direction, packet.ID) error

	// AllPackets will return all packets currently saved in the session.
	AllPackets(clientsession.Direction) ([]packet.GenericPacket, error)
...
func (c *Client) PublishMessage(msg *packet.Message) (GenericFuture, error) {
	...
	if msg.QOS > 0 {
		err := c.Session.SavePacket(clientsession.Outgoing, publish)
		if err != nil {
			return nil, c.cleanup(err, true, false)
		}
	}
...
func (c *Client) processConnack(connack *packet.ConnackPacket) error {
	...
	packets, err := c.Session.AllPackets(clientsession.Outgoing)
	if err != nil {
...
func (c *Client) processSuback(suback *packet.SubackPacket) error {
	...
	err := c.Session.DeletePacket(clientsession.Outgoing, suback.ID)
	if err != nil {
		return err
	}
...
func (c *Client) processUnsuback(unsuback *packet.UnsubackPacket) error {
	// remove packet from store
	err := c.Session.DeletePacket(clientsession.Outgoing, unsuback.ID)
	if err != nil {
		return err
	}
...
func (c *Client) processPublish(publish *packet.PublishPacket) error {
	...
	if publish.Message.QOS == 2 {
		// store packet
		err := c.Session.SavePacket(clientsession.Incoming, publish)
		if err != nil {
			return c.die(err, true, false)
		}
...
func (c *Client) processPubackAndPubcomp(id packet.ID) error {
	// remove packet from store
	err := c.Session.DeletePacket(clientsession.Outgoing, id)
	if err != nil {
		return err
	}
...
func (c *Client) processPubrec(id packet.ID) error {
	...
	err := c.Session.SavePacket(clientsession.Outgoing, pubrel)
	if err != nil {
		return c.die(err, true, false)
	}
...
func (c *Client) processPubrel(id packet.ID) error {
	...
	pkt, err := c.Session.LookupPacket(clientsession.Incoming, id)
	if err != nil {
		return c.die(err, true, false)
	}
	...
	// remove packet from store
	err = c.Session.DeletePacket(clientsession.Incoming, id)
	if err != nil {
		return c.die(err, true, false)
	}
